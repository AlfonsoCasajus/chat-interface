...
const conversations = computed(
	() => store.getters["conversations/conversations"]
);
const selectedConversation = computed(
	() => store.getters["conversations/selectedConversation"]
);
	...
	<div class="h-screen flex flex-col">
		...
		<div class="h-1 flex justify-between flex-grow max-lg:flex-col">
			<span
				...
				:class="[
					'min-w-[400px] max-lg:w-full w-1/4 overflow-y-auto h-full dark:bg-gray-800',
					{ 'max-lg:hidden': selectedConversation }
				]"
				@scroll="handleScroll"
			>
				<Conversations
			...
			<span
				:class="[
					'h-full w-full',
					{ 'max-lg:hidden': !selectedConversation || isCustomerVisible }
				]"
			>
				<Chat
					:conversation="selectedConversation"
					@send-message="updateConversation"
				>
			...
			<span
				v-if="selectedConversation && isCustomerVisible"
				class="min-w-80 h-full max-lg:w-full"
			>
				<CustomerDetail
					:conversation="selectedConversation"
					@close="toggleCustomerDetail"
				/>
			</span>
		</div>
	</div>
