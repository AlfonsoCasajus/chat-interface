<template>
	<div
		v-if="conversation && firstMessageSended && lastMessageSended"
		:class="[
			'flex',
			'items-center',
			'p-4',
	...
	>
		...
		<div class="flex flex-col flex-grow">
			...
			<p
				class="text-gray-700 text-sm truncate dark:text-gray-300 text-ellipsis"
			>
				{{ lastMessageSended.message }}
			</p>
			<p class="text-gray-500 text-xs dark:text-gray-400">
				{{ lastMessageSended.time }}
			</p>
			<div class="flex items-center justify-between mt-2">
				<span
					class="bg-gray-100 text-gray-600 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300"
					>{{ conversation.agent }}</span
				>
				<span
					class="bg-gray-100 text-gray-600 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300"
					>{{ conversation.sector }}</span
				>
				<time class="text-gray-500 text-xs dark:text-gray-400">{{
					firstMessageSended.time
				}}</time>
			</div>
		</div>
	</div>
